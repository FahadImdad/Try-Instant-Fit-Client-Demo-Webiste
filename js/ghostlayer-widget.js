"use strict";(()=>{var h="https://backend-tryinstantfit.vercel.app",b=class{constructor(e){this.config=null;this.overlayRoot=null;this.overlayShadow=null;this.currentProduct=null;this.isImageOverlay=!1;this.cameraStream=null;this.selectedFile=null;this.countdownTimer=null;this.lightingInterval=null;this.lastBrightness=128;this.brandId=e,this.init()}async init(){try{if(console.log("[GhostLayer] Initializing for brand:",this.brandId),await this.loadConfiguration(),this.config?.buttonColor&&document.documentElement.style.setProperty("--gl-brand-color",this.config.buttonColor),!this.config?.enabled){console.log("[GhostLayer] Widget disabled");return}if(!this.isProductPage()){console.log("[GhostLayer] Not a product page");return}if(this.currentProduct=this.detectProduct(),!this.currentProduct){console.log("[GhostLayer] No product detected");return}console.log("[GhostLayer] Product:",this.currentProduct.name),this.injectButton(),this.trackEvent("widget_loaded",{product_id:this.currentProduct.id})}catch(e){console.error("[GhostLayer] Init error:",e)}}async loadConfiguration(){try{let e=await fetch(`${h}/api/widget/config/${this.brandId}`,{headers:{Accept:"application/json"}});if(e.ok){this.config=await e.json();return}}catch{}this.config={brandId:this.brandId,apiEndpoint:h,buttonText:"Try It On \u2728",buttonColor:"#1a1a2e",buttonPosition:"bottom-right",enabled:!0}}isProductPage(){for(let d of Array.from(document.querySelectorAll('script[type="application/ld+json"]')))try{let l=JSON.parse(d.textContent||"");if((Array.isArray(l)?l:[l]).some(c=>c["@type"]==="Product"))return!0}catch{}if((document.querySelector('meta[property="og:type"]')?.getAttribute("content")||"").toLowerCase().includes("product"))return!0;let t=window.location.pathname.toLowerCase(),r=window.location.search.toLowerCase();if(["/product/","/products/","/p/","/item/","/shop/","/clothing/","product.html"].some(d=>t.includes(d))||r.includes("id=")&&t.includes("product"))return!0;let n=!!(document.querySelector('[class*="price"]')||document.querySelector('[itemprop="price"]')),o=!!(document.querySelector('[class*="add-to-cart"]')||document.querySelector('[class*="addtocart"]')||document.querySelector('button[name="add"]')),a=!!(document.querySelector('[class*="product-image"]')||document.querySelector('[class*="product-gallery"]'));return n&&(o||a)}detectProduct(){return this.fromSchema()||this.fromOG()||this.fromDOM()}fromSchema(){for(let e of Array.from(document.querySelectorAll('script[type="application/ld+json"]')))try{let t=JSON.parse(e.textContent||""),i=(Array.isArray(t)?t:[t]).find(o=>o["@type"]==="Product");if(!i)continue;let n=Array.isArray(i.image)?i.image[0]:i.image;if(!n)continue;return{id:i.sku||i["@id"]||this.genId(),name:i.name||document.title,imageUrl:n,price:i.offers?.price?.toString(),url:location.href}}catch{}return null}fromOG(){let e=document.querySelector('meta[property="og:image"]')?.getAttribute("content"),t=document.querySelector('meta[property="og:title"]')?.getAttribute("content");return e&&t?{id:this.genId(),name:t,imageUrl:e,url:location.href}:null}fromDOM(){let e=["#pd-main-img","#product-featured-image",".product-image img",".product-gallery img",".product-single__media img",".product-featured-image img",".woocommerce-product-gallery__image img","[data-product-image] img",'[class*="pd-main"] img','[class*="product"] img',"main img"],t="";for(let n of e){let o=document.querySelector(n);if(!o)continue;let a=o.tagName==="IMG"?o:o.querySelector("img");if(a?.src&&!this.isPlaceholder(a.src)){t=a.src;break}}let r=["#pd-name",'h1[class*="product"]','[class*="product-title"]','[class*="product-name"]','[itemprop="name"]',"h1"],i="";for(let n of r){let o=document.querySelector(n)?.textContent?.trim();if(o){i=o;break}}return t&&i?{id:this.genId(),name:i,imageUrl:t,url:location.href}:null}isPlaceholder(e){let t=e.toLowerCase();return t.includes("placeholder")||t.includes("no-image")||t.includes("noimage")||t.includes("icon")||t.includes("logo")}genId(){return btoa(location.pathname).replace(/[^a-zA-Z0-9]/g,"").substring(0,16)}findProductImg(){let e=["#pd-main-img","#product-featured-image",".product-featured-image img",".product-single__media img",".product-image img",".woocommerce-product-gallery__image img",'[class*="product-gallery"] img','[class*="product-image"] img','[class*="product-photo"] img','[class*="product-media"] img'];for(let t of e){let r=document.querySelector(t);if(r instanceof HTMLImageElement&&r.src&&!this.isPlaceholder(r.src))return r}return null}injectButton(){let e=this.config?.buttonColor||"#1a1a2e",t=this.config?.buttonText||"Try It On \u2728",r=document.createElement("div");r.id="ghostlayer-btn-root";let i=this.findProductImg();if(i){this.isImageOverlay=!0;let o=i.parentElement;o?(getComputedStyle(o).position==="static"&&(o.style.position="relative"),Object.assign(r.style,{position:"absolute",top:"14px",right:"14px",zIndex:"100",pointerEvents:"auto"}),o.appendChild(r)):(document.body.appendChild(r),this.isImageOverlay=!1)}else document.body.appendChild(r),this.isImageOverlay=!1;let n=r.attachShadow({mode:"open"});if(this.isImageOverlay)n.innerHTML=`
        <style>
      ...
      `;else{let o=this.config?.buttonPosition||"bottom-right",a={"bottom-right":"bottom:24px;right:24px;","bottom-left":"bottom:24px;left:24px;","top-right":"top:24px;right:24px;","top-left":"top:24px;left:24px;"};n.innerHTML=`
        ...
        <button id="gl-try-btn">${t}</button>
      `}n.getElementById("gl-try-btn")?.addEventListener("click",()=>this.openOverlay()),this.injectOverlayRoot()}injectOverlayRoot(){this.overlayRoot||(this.overlayRoot=document.createElement("div"),this.overlayRoot.id="ghostlayer-overlay-root",document.body.appendChild(this.overlayRoot),this.overlayShadow=this.overlayRoot.attachShadow({mode:"open"}))}openOverlay(){this.trackEvent("tryon_opened",{product_id:this.currentProduct?.id}),this.selectedFile=null,this.renderOverlay("upload")}openForProduct(e){this.config?.enabled&&(this.overlayRoot||this.injectOverlayRoot(),this.currentProduct=e,this.openOverlay())}renderOverlay(e,t){if(!this.overlayShadow)return;let r=this.overlayShadow.getElementById("gl-overlay");r&&r.remove();let i=document.createElement("div");i.id="gl-overlay",i.innerHTML=this.getOverlayInner(e,t),this.overlayShadow.appendChild(i),this.bindOverlayEvents(e)}getOverlayInner(e,t){let r=this.currentProduct?.name||"this item",i=this.currentProduct?.imageUrl||"",o={upload:`
        <div class="gl-card">
          <div class="gl-header">
...
<div class="gl-countdown-overlay" id="gl-countdown-overlay" style="display:none">
    ...
    `}bindOverlayEvents(e){let t=this.overlayShadow;if(t){if(t.getElementById("gl-close")?.addEventListener("click",()=>this.closeOverlay()),t.getElementById("gl-overlay")?.addEventListener("click",r=>{r.target.id==="gl-overlay"&&this.closeOverlay()}),e==="upload"){let r=t.getElementById("gl-generate-btn"),i=s=>{this.selectedFile=s,r&&(r.disabled=!1)},n=t.getElementById("gl-tab-upload"),o=t.getElementById("gl-tab-camera"),a=t.getElementById("gl-panel-upload"),d=t.getElementById("gl-panel-camera");n?.addEventListener("click",()=>{this.stopCamera(),n.classList.add("gl-tab-active"),o?.classList.remove("gl-tab-active"),a&&(a.style.display="block"),d&&(d.style.display="none")}),o?.addEventListener("click",async()=>{o.classList.add("gl-tab-active"),n?.classList.remove("gl-tab-active"),a&&(a.style.display="none"),d&&(d.style.display="block"),await this.startCamera(t)});let l=t.getElementById("gl-upload-zone"),g=t.getElementById("gl-file-input"),c=t.getElementById("gl-upload-preview-wrap"),u=t.getElementById("gl-upload-preview-img"),p=s=>{if(!s.type.startsWith("image/"))return;i(s);let m=URL.createObjectURL(s);u&&(u.src=m),l&&(l.style.display="none"),c&&(c.style.display="block")};l?.addEventListener("click",()=>g?.click()),t.getElementById("gl-change-photo")?.addEventListener("click",()=>{this.selectedFile=null,r&&(r.disabled=!0),l&&(l.style.display="block"),c&&(c.style.display="none"),g?.click()}),g?.addEventListener("change",()=>{let s=g.files?.[0];s&&p(s)}),l?.addEventListener("dragover",s=>{s.preventDefault(),l.classList.add("gl-drag-over")}),l?.addEventListener("dragleave",()=>l.classList.remove("gl-drag-over")),l?.addEventListener("drop",s=>{s.preventDefault(),l.classList.remove("gl-drag-over");let m=s.dataTransfer?.files[0];m&&p(m)}),t.getElementById("gl-capture-now")?.addEventListener("click",()=>{this.capturePhoto(t,i)}),t.getElementById("gl-timer-btn")?.addEventListener("click",()=>{this.startCountdown(t,5,i)}),t.getElementById("gl-retake-btn")?.addEventListener("click",async()=>{this.selectedFile=null,r&&(r.disabled=!0);let s=t.getElementById("gl-camera-preview-wrap"),m=t.getElementById("gl-camera-wrap"),f=t.getElementById("gl-camera-controls"),y=t.getElementById("gl-capture-now"),v=t.getElementById("gl-timer-btn"),x=t.getElementById("gl-lighting-bar");s&&(s.style.display="none"),m&&(m.style.display="block"),f&&(f.style.display="flex"),x&&(x.style.display="flex"),y&&(y.disabled=!0),v&&(v.disabled=!0),await this.startCamera(t)}),r?.addEventListener("click",async()=>{this.selectedFile&&await this.generateTryOn(this.selectedFile)})}e==="result"&&(t.getElementById("gl-retry")?.addEventListener("click",()=>{this.selectedFile=null,this.renderOverlay("upload")}),t.getElementById("gl-buy-btn")?.addEventListener("click",()=>{this.trackEvent("buy_clicked",{product_id:this.currentProduct?.id}),this.closeOverlay(),(document.querySelector('button[name="add"]')||document.querySelector('[class*="add-to-cart"]'))?.click()})),e==="error"&&t.getElementById("gl-retry")?.addEventListener("click",()=>{this.selectedFile=null,this.renderOverlay("upload")})}}async startCamera(e){let t=e.getElementById("gl-camera-video"),r=e.getElementById("gl-camera-error"),i=e.getElementById("gl-capture-now"),n=e.getElementById("gl-timer-btn"),o=e.getElementById("gl-camera-wrap");this.stopCamera();try{let a=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:640},height:{ideal:480}}});this.cameraStream=a,t&&(t.srcObject=a),o&&(o.style.display="block"),r&&(r.style.display="none"),this.monitorLighting(e)}catch{r&&(r.style.display="block"),o&&(o.style.display="none"),i&&(i.disabled=!0),n&&(n.disabled=!0)}}capturePhoto(e,t){let r=e.getElementById("gl-camera-video"),i=e.getElementById("gl-camera-canvas");if(!r||!i||!r.videoWidth)return;i.width=r.videoWidth,i.height=r.videoHeight;let n=i.getContext("2d");if(n){if(this.lastBrightness>0&&this.lastBrightness<80){let o=Math.min(2,100/Math.max(this.lastBrightness,20));n.filter=`brightness(${o.toFixed(2)})`}n.drawImage(r,0,0),n.filter="none",this.stopCamera(),i.toBlob(o=>{if(!o)return;let a=new File([o],"selfie.jpg",{type:"image/jpeg"}),d=e.getElementById("gl-camera-wrap"),l=e.getElementById("gl-camera-controls"),g=e.getElementById("gl-lighting-bar"),c=e.getElementById("gl-camera-preview-wrap"),u=e.getElementById("gl-camera-preview-img"),p=URL.createObjectURL(a);u&&(u.src=p),d&&(d.style.display="none"),l&&(l.style.display="none"),g&&(g.style.display="none"),c&&(c.style.display="block"),t(a)},"image/jpeg",.92)}}monitorLighting(e){this.lightingInterval&&(clearInterval(this.lightingInterval),this.lightingInterval=null);let t=e.getElementById("gl-camera-video"),r=e.getElementById("gl-lighting-dot"),i=e.getElementById("gl-lighting-text"),n=e.getElementById("gl-capture-now"),o=e.getElementById("gl-timer-btn"),a=document.createElement("canvas");a.width=64,a.height=48;let d=a.getContext("2d");this.lightingInterval=window.setInterval(()=>{if(!d||!t||!t.videoWidth)return;d.drawImage(t,0,0,64,48);let l=d.getImageData(0,0,64,48).data,g=0;for(let s=0;s<l.length;s+=4)g+=.299*l[s]+.587*l[s+1]+.114*l[s+2];let c=g/(64*48);this.lastBrightness=c;let u=c>=80,p=c>=50;r&&(r.style.background=u?"#22c55e":p?"#f59e0b":"#ef4444"),i&&(i.textContent=u?"\u2713 Good lighting":p?"\u26A0 Brighter light will improve results":"\u2717 Too dark \u2014 move to better lighting"),this.countdownTimer||(n&&(n.disabled=!p),o&&(o.disabled=!p))},500)}startCountdown(e,t,r){let i=e.getElementById("gl-countdown-overlay"),n=e.getElementById("gl-countdown-num"),o=e.getElementById("gl-capture-now"),a=e.getElementById("gl-timer-btn");this.countdownTimer&&(clearInterval(this.countdownTimer),this.countdownTimer=null),i&&(i.style.display="flex"),o&&(o.disabled=!0),a&&(a.disabled=!0);let d=t;n&&(n.textContent=String(d)),this.countdownTimer=window.setInterval(()=>{d--,n&&(n.textContent=String(d)),d<=0&&(clearInterval(this.countdownTimer),this.countdownTimer=null,i&&(i.style.display="none"),o&&(o.disabled=!1),a&&(a.disabled=!1),this.capturePhoto(e,r))},1e3)}stopCamera(){this.countdownTimer&&(clearInterval(this.countdownTimer),this.countdownTimer=null),this.lightingInterval&&(clearInterval(this.lightingInterval),this.lightingInterval=null),this.cameraStream&&(this.cameraStream.getTracks().forEach(e=>e.stop()),this.cameraStream=null)}closeOverlay(){this.stopCamera(),this.overlayShadow?.getElementById("gl-overlay")?.remove(),this.trackEvent("tryon_closed",{product_id:this.currentProduct?.id})}async generateTryOn(e){this.stopCamera(),this.renderOverlay("processing"),this.trackEvent("tryon_started",{product_id:this.currentProduct?.id});try{let t=new FormData;t.append("user_photo",e),t.append("product_image_url",this.currentProduct?.imageUrl||""),t.append("product_id",this.currentProduct?.id||""),t.append("product_name",this.currentProduct?.name||""),t.append("brand_id",this.brandId),t.append("source","ghost-layer"),t.append("provider","gemini");let r=this.config?.apiEndpoint||h,i=await fetch(`${r}/api/widget/try-on`,{method:"POST",body:t});if(!i.ok){let a=await i.json().catch(()=>({}));throw new Error(a.debug||a.error||`Server error: ${i.status}`)}let n=await i.json(),o=n.result_url||n.resultUrl;if(!o)throw new Error("No result image returned");this.trackEvent("tryon_completed",{product_id:this.currentProduct?.id,result_url:o}),this.renderOverlay("result",{resultUrl:o})}catch(t){console.error("[GhostLayer] Try-on failed:",t),this.trackEvent("tryon_failed",{product_id:this.currentProduct?.id,error:String(t)}),this.renderOverlay("error",{errorMsg:t instanceof Error?t.message:"Please try again."})}}async trackEvent(e,t){try{let r=this.config?.apiEndpoint||h;await fetch(`${r}/api/widget/track`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({brand_id:this.brandId,event_name:e,event_data:t,page_url:location.href,timestamp:new Date().toISOString()}),keepalive:!0})}catch{}}};(function(){let e=(document.currentScript||document.querySelector('script[src*="ghostlayer-widget"]'))?.getAttribute("data-brand-id");if(!e){console.warn("[GhostLayer] No data-brand-id found on script tag");return}let t=()=>setTimeout(()=>{let r=new b(e);window.__ghostlayer=r},0);document.readyState==="loading"?document.addEventListener("DOMContentLoaded",t):t()})();})();
